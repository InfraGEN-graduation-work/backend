...
먼저 `POST /api/v1/auth/email/code`에 `{"email":"user@example.com"}`을 보내 인증 메일을 요청한다.
성공 응답은 HTTP 200, code `AUTH200_4`, result `null`이며 인증번호는 응답에 포함하지 않는다.
이미 가입된 이메일도 같은 HTTP 200 / `AUTH200_4` 응답을 반환하며 인증 메일은 발송하지 않는다.
따라서 이 API 응답으로 이메일 가입 여부를 판단하지 않는다.
메일로 받은 숫자 6자리를 입력한 후 회원가입 form을 제출한다.

인증번호는 5분간 유효하며 가입 시 한 번만 사용할 수 있다. 재발송은 30초 간격, 이메일당 시간당 최대 10회다.
재발송하면 기존 번호는 무효화되고, 같은 번호를 5회 틀리면 새 번호를 받아야 한다.
인증번호는 앞자리 0을 보존하도록 문자열로 전송한다.
...
- `AUTH400_3`은 번호 불일치·만료·이미 사용한 번호, `AUTH429_1`은 발송/입력 횟수 제한이다.
- `AUTH503_1`은 메일 발송 실패다. 실패한 발송도 재발송 제한에 포함된다.
- 가입 저장 실패 시 번호가 이미 소비됐을 수 있으므로 새 인증번호를 요청한다.
...
용도: 읽기 권한이 있는 사용자의 프로젝트 정보와 저장된 node·edge를 캔버스에 복원한다. owner·EDITOR·VIEWER가 사용할 수 있으며, 쓰기 권한은 별도 API 정책을 따른다.
...
- 권한이 없거나 존재하지 않으면 404로 프로젝트 목록으로 돌아간다.
