...
public class NginxParser implements ComponentParser {
    ...
    public BaseComponent parse(NodeDTO node, JsonNode props, int port) {
        ...
        String webRootVolumeName = props.path("webRootVolumeName").asString();

        if (imageVersion == null || imageVersion.isBlank()) {
            throw new ParsingException(ParsingErrorCode.MISSING_NGINX_IMAGE_VERSION);
        }
        ...
        return NginxComponent.builder()
                ...
                .webRootVolumeName(webRootVolumeName != null ? webRootVolumeName : "")
                .build();
    }
}
